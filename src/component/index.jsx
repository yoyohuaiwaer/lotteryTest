    ...
    var historyList = this.state.item.map(function (m, i) {
      ...
      if (i < 3) {
        return (<li key={m.opentimestamp}>
          ...
          </time> <span className='hisNum' > {num} </span></li>
        )
      }
    })
      ...
      <div className='main' >
        <div className='content1 clearfix'>
          <div className='countDownBox'>
            <div className='issue'>第 <span> {issue} </span>期截止时间</div>
            <div className="countdown" > {this.state.text_hh}: {this.state.text_mm}: {this.state.text_ss}</div>
          </div>
          <div className='opencode' >
            <div className='issue' >
              <span className='chooseType' > 11 选5 </span>第<span>{issue}</span> 期号码
            </div>
            <ul className=''> {boxnum} </ul> 
          </div>
          <div className='history' >
            <ul className='' >
              <li>
                <time className='hisDate'>期次 </time><span className='hisNum'>开奖号码</span>
              </li>
               {historyList} 
            </ul>
          </div>
